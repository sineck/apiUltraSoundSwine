...
?????? repo ???? `model/` ??? `AnomalyDetection/artifacts/models/` ???? ????????????? **???? B1: ????? Image** ???? clone repo ???? build ????????? ???????????? model artifacts ???????????????

- `model/` ????????? YOLO PDF classifier ???? `best_finetune_YOLO26-cls_Ver2_20260424.pt`
- `AnomalyDetection/artifacts/models/` ????????? `model_registry.json` ??? `.joblib` anomaly artifacts

???? `config/.env` ??????? runtime file ???????????????????????? ???????????????? secret ????????? Git

?????? **???? B2: Image Transfer** ????????????????????????????????????????????????????????????? ????????????????? image ????????????????????????.
